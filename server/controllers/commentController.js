 ...
 * @desc    Authenticated users: post a comment on a campaign
 */
 ...
 * @desc    List comments for a campaign (public)
 */
export const getComments = asyncHandler(async (req, res) => {
  ...
  const [comments, total] = await Promise.all([
    Comment.find({ campaign: req.params.campaignId })
      .populate('user', 'name avatar')
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(Number(limit)),
    Comment.countDocuments({ campaign: req.params.campaignId }),
  ]);
